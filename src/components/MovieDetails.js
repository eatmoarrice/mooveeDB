...
export default function MovieDetails(props) {
	...
	const getJustOneMovie = async () => {
		try {
			let url = `https://api.themoviedb.org/3/movie/${urlParams.id}?api_key=${apiKey}&language=en-US`;
	...
	const getVideo = async () => {
		try {
			let url = `https://api.themoviedb.org/3/movie/${urlParams.id}/videos?api_key=${apiKey}&language=en-US`;
			let data = await fetch(url);
			if (data.status !== 200) {
				throw new Error('data is wrong');
			}
